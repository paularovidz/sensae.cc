<header data-header class="header group/header mt-5 transform sticky-header" x-data="{ mobileOpen: false }">
                ...
                @foreach(($menu['entries'] ?? []) as $entry)
                    ...
                    @else
                        <div x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false" class="relative">
                            <button @click="open = !open" class="nav-link text-white transition rounded-full hover:bg-white/5 flex items-center gap-1">
                                {{ $entry['label'] }}
