...
// Stamps for paid sessions (séances à payer) + 1 for free session
...
// Total stamps including free session box
const totalStampsWithBonus = computed(() => {
  return stamps.value.length + 1
})

// Grid columns based on total (max 5 per row on desktop, adapt for mobile)
const gridColumns = computed(() => {
  const total = totalStampsWithBonus.value
  ...
  <div class="card-dark overflow-hidden">
    ...
    <div class="card-dark-body">
      ...
      <template v-else>
        ...
        <div class="mb-6">
          <!-- All stamps in one grid -->
          <div class="grid gap-2" :style="{ gridTemplateColumns: `repeat(${gridColumns}, minmax(0, 1fr))` }">
            ...
            <!-- Free session box (bonus) - inside the grid -->
            <div
              :class="[
                'aspect-square rounded-xl flex flex-col items-center justify-center font-semibold transition-all duration-300',
                freeSessionUnlocked
            ...
            >
              <!-- Gift icon -->
              <svg v-if="freeSessionUnlocked" class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
              <svg v-else-if="loyaltyData.free_session_used_at" class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20">
                <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
              </svg>
              ...
              <span class="hidden sm:block text-xs mt-1">Offerte</span>
            </div>
          </div>
        </div>
